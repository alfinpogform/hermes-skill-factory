...
def generate_skill_md(
    ...
    # Dedent the static template *before* substituting in multi-line values —
    # textwrap.dedent looks at the common leading whitespace of the final
    # string, so if it ran after interpolation, the flush-left lines inside
    # steps_md/examples_md would collapse the common prefix to "" and no
    # indentation would be stripped at all.
...
def generate_plugin_py(
    ...
    display_name = skill_name.replace("-", " ").title()
    command_fn_name = f"run_{skill_name.replace('-', '_')}"

    # Build the step comments already indented to the column they need to
    # land at inside the generated function body (see the note in
    # generate_skill_md about why dedent can't run *after* interpolation).
    steps_comments = "\n".join(f"# Step {i + 1}: {s}" for i, s in enumerate(workflow_steps))
    steps_comments = textwrap.indent(steps_comments, " " * 8)
    ...
    template = textwrap.dedent('''\
        ...
        def register(hermes):
            ...
            async def {command_fn_name}(ctx, args: str = ""):
    ...
    content = template.format(
        ...
        command_fn_name=command_fn_name,
